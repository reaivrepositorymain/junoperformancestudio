...
export default function BigCalendar({ meetings }: { meetings: any[] }) {
  const today = moment();
    ...
    <div className="flex-1 w-full px-2 mx-auto mb-4">
      ...
      <div className="flex items-center justify-between w-full mb-2">
        <div className="flex items-center gap-2">
          {view === "month" && (
            <>
              <button onClick={prevMonth} className="p-2 rounded-lg bg-gray-100 hover:bg-[#E84912]/10 text-[#E84912] transition shadow" aria-label="Previous Month">
                <ChevronLeft />
              </button>
              <span className="text-2xl font-cursive font-semibold text-gray-900 tracking-wide drop-shadow">
                {currentMonth.format("MMMM YYYY")}
              </span>
              <button onClick={nextMonth} className="p-2 rounded-lg bg-gray-100 hover:bg-[#E84912]/10 text-[#E84912] transition shadow" aria-label="Next Month">
                <ChevronRight />
              </button>
            </>
          )}
        ...
        <>
          <div className="grid grid-cols-7 gap-0 w-full border-2 rounded-t-sm mt-3 border-gray-200 overflow-hidden bg-white/80 shadow">
            {["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"].map((d) => (
              <div key={d} className="text-center font-cursive tracking-normal text-lg font-semibold text-[#E84912] py-2 border-r last:border-r-0 border-gray-200 bg-white/90">
                {d}
              </div>
            ))}
          </div>
      ...
      {view === "week" && (
        <>
          <div className="grid grid-cols-7 gap-0 w-full mt-3 rounded-t-sm border-2 border-gray-200 overflow-hidden bg-white/80 shadow">
            {["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"].map((d) => (
              <div key={d} className="text-center font-cursive tracking-normal font-semibold text-[#E84912] py-2 border-r last:border-r-0 border-gray-200 bg-white/90">
                {d}
              </div>
            ))}
          </div>
